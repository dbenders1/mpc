...
StaticPolyhedronConstraints::StaticPolyhedronConstraints(std::string name, ros::NodeHandle &nh, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, DataSaverJson *ptr_data_saver_json, RealTimeData &data): 
    ControllerModule(ModuleType::CONSTRAINT, name, nh, ptr_config, ptr_solver, ptr_data_saver_json)
{
    MPC_WARN_ALWAYS("Initializing " << name_ << " module");

    // Constraints variables
    // NOTE: this assumes the initial state is not constrained (in total N circular/ellipsoidal paths are constructed), but stored from the previous run for visualization
    n_stages_to_construct_ = ptr_solver_->n_;
    n_constraints_per_stage_ = ptr_config_->n_constraints_per_stage_;
    constraints_ = Eigen::MatrixXd::Zero(3*n_constraints_per_stage_, n_stages_to_construct_+1);
    ...
    constraints_to_rec_and_vis_ = Eigen::MatrixXd::Zero(3*n_constraints_per_stage_, n_stages_to_rec_and_vis_);
    n_relevant_constraints_to_rec_and_vis_ = Eigen::VectorXi::Zero(n_stages_to_rec_and_vis_);
    if (ptr_config_->record_constraints_)
    {
        constraints_rec_pub_ = nh.advertise<mpc_msgs::ConstraintsOverHorizon>(ptr_config_->constraints_rec_topic_, 1);
    ...
    std::vector<int> idc = ptr_solver_->returnDataPositionsState({"x","y","psi"});
    x_position_ = idc[0];
    y_position_ = idc[1];
    psi_position_ = idc[2];
    ...
    // Only initialize the polyhedron constraints generator in single-layer or highest-layer MPC
    if (ptr_config_->layer_idx_ == ptr_config_->n_layers_-1)
    {
        ...
        case -1:
            n_path_ = n_stages_to_construct_+1;  // one extra stage needed, because line segments lie in between
            break;
        ...
        case 0:
            n_path_ = 2*n_stages_to_construct_;  // each stage needs two points in space
            break;
        ...
        default:
            MPC_WARN("StaticPolyhedronConstraints: Unknown value for 'ptr_config_->line_segment_to_stage_' (should be in {-1,0,1}). Setting 1 as value");
            ptr_config_->line_segment_to_stage_ = 1;
            n_path_ = n_stages_to_construct_+1;  // one extra stage needed, because line segments lie in between
            break;
        }
        // Do not reserve memory for path_ and poly_constraints_ -> this gives runtime errors or malfunctioning behaviour, probably caused by the dynamically-sized Eigen matrices and vectors

        // Only look around for obstacles using a box with sides of width 2*bounding_box_width_
        decomp_util_.set_local_bbox(Vec2f(ptr_config_->bounding_box_width_, ptr_config_->bounding_box_width_));

        ros_markers_.reset(new ROSMarkerPublisher(nh, ptr_config_->constraints_vis_topic_.c_str(), ptr_config_->robot_target_frame_, (ptr_solver_->n_+2)*(2*ptr_config_->n_constraints_per_stage_+1)));

        local_map_sub_ = nh.subscribe(ptr_config_->occ_grid_topic_, 1, &StaticPolyhedronConstraints::OccGridMapCallBack, this);

        // This is the radius around the bounding box that is used to determine the local map distance
        radius_around_bbox_ = 0.5*std::sqrt(2 * 2*ptr_config_->bounding_box_width_ * 2*ptr_config_->bounding_box_width_);

        occ_pos_vec_stages_.resize(n_path_-1);

        // Create publisher local gridmap
        local_occ_grid_pub_ = nh.advertise<nav_msgs::OccupancyGrid>(ptr_config_->local_occ_grid_topic_, 1);
    }
    else
    {
        // Memory for received_constraints_ and received_n_relevant_constraints_ can only be reserved when first receiving the constraints, since size is unknown here

        // Set validity of real-time data to zero until valid data is received for all lower-layer MPCs
        n_it_valid_ = 0;
        n_it_valid_max_ = 0;

        ros_markers_.reset(new ROSMarkerPublisher(nh, ptr_config_->constraints_vis_topic_.c_str(), ptr_config_->robot_target_frame_, (ptr_solver_->n_+1)*(2*ptr_config_->n_constraints_per_stage_+1)));
    }

    MPC_WARN_ALWAYS(name_ << " module initialized");
}

void StaticPolyhedronConstraints::OnDataReceived(RealTimeData &data, std::string data_name)
{
    PROFILE_AND_LOG_INFO("StaticPolyhedronConstraints::OnDataReceived");

    if ((ptr_config_->layer_idx_ == ptr_config_->n_layers_-1)){ return;}

    // Save correct range of received_constraints to use + reduce real-time data validity for all lower-layer MPCs
    if (data_name == "Constraints")
    {
        // When receiving constraints for the first time, make sure to allocate the proper amount of memory for the received constraints
        if (!first_constraints_received_)
        {
            received_n_stages_ = data.constraints_.cols();
            received_constraints_ = Eigen::MatrixXd::Zero(3*n_constraints_per_stage_, received_n_stages_);
            received_n_relevant_constraints_ = Eigen::VectorXi::Zero(received_n_stages_);
...
void StaticPolyhedronConstraints::Update(RealTimeData &data)
{
    ...
    if (ptr_config_->layer_idx_ == ptr_config_->n_layers_-1)
    {
        // First shift the constraints and number of relevant constraints, such that the constraints for stage 0 (not used in the solver) are set equal to the constraints of stage 1
        ...
        {
            MPC_ERROR("StaticPolyhedronConstraints: Line segments computation failed!");
        }
        ...
        // Initially, set the constraints for stage 0 equal to the constraints of stage 1
        ...
        for (int k = 1; k < n_stages_to_construct_+1; k++)
        {
            ...
            // If more constraints fields are given in the solver: insert dummies (x constraints far away, see above)
            for (int l = n_relevant_constraints_(k); l < n_constraints_per_stage_; l++)
            {
                constraints_(3*l,k) = 1;
                constraints_(3*l+1,k) = 0;
                // Note: using stage_vars_ of previous run, so need account for stage index N here
                if (k == n_stages_to_construct_) constraints_(3*l+2,k) = ptr_solver_->stage_vars_(x_position_,k) + 100;
                else constraints_(3*l+2,k) = ptr_solver_->stage_vars_(x_position_,k+1) + 100;
            }
    ...
    // Process received constraints and reduce received data validity for all lower-layer MPCs
    else if (ptr_config_->layer_idx_ < ptr_config_->n_layers_-1)
    {
        ...
        for (int k = 0; k < n_stages_to_construct_+1; k++)
        {
            ...
            n_relevant_constraints_(k) = received_n_relevant_constraints_(constraint_idx);
        }
...
void StaticPolyhedronConstraints::TightenConstraints(double c_o, double alpha)
{
    PROFILE_AND_LOG_INFO("StaticPolyhedronConstraints::TightenConstraints");

    // Tighten relevant constraints
    // NOTE: only tighten the constructed constraints, since these stages will not be communicated to the interface
    for (int k = 1; k < n_stages_to_construct_+1; k++)
    {
        for (int l = 0; l < n_relevant_constraints_(k); l++)
        {
            constraints_(3*l+2,k) = constraints_(3*l+2,k) - c_o * alpha;
        }
    }
}
...
void StaticPolyhedronConstraints::SetParameters(const RealTimeData &data)
{
    ...
    // For stage 0, insert constraints that the solver does not like as check for correct usage of constraints over the horizon
    ptr_solver_->par_constraints_.block(disc0_0_a1_position_,0,3*n_constraints_per_stage_,1) = Eigen::VectorXd::Zero(3*n_constraints_per_stage_);
    for (int k = 1; k < ptr_solver_->n_+1; k++)
    {
        ptr_solver_->par_constraints_.block(disc0_0_a1_position_,k,3*n_constraints_per_stage_,1) = constraints_.col(k);
    }
}

void StaticPolyhedronConstraints::SetConstraintsToRecAndVis(const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints)
{
    PROFILE_AND_LOG_INFO("StaticPolyhedronConstraints::SetConstraints");

    use_ext_con_to_rec_and_vis_ = true;

    // Reserve memory for given the known amount of stages for which to visualize the constraints
...
void StaticPolyhedronConstraints::PublishData(const mpc_msgs::MpcHeader &mpc_header)
{
    if (ptr_config_->record_constraints_)
    {
        ...
        constraints_msg_.con.resize(3*n_constraints_per_stage_*n_stages_to_rec_and_vis_);
        ...
        for (int k = 0; k < n_stages_to_rec_and_vis_; k++)
        {
            if (use_ext_con_to_rec_and_vis_)
            {
                ...
                {
                    for (int m = 0; m < 3; m++) constraints_msg_.con[k*n_constraints_per_stage_*3+l*3+m] = constraints_to_rec_and_vis_(3*l+m,k);
                }
                constraints_msg_.n_relevant_con[k] = n_relevant_constraints_to_rec_and_vis_(k);
            }
            else
            {
                for (int l = 0; l < n_constraints_per_stage_; l++)
                {
                    for (int m = 0; m < 3; m++) constraints_msg_.con[k*n_constraints_per_stage_*3+l*3+m] = constraints_(3*l+m,k);
                }
...
{
    if (!ptr_config_->draw_constraints_){return;}

    if (use_ext_con_to_rec_and_vis_) CreateVisualizations(constraints_to_rec_and_vis_, n_relevant_constraints_to_rec_and_vis_, n_stages_to_rec_and_vis_);
    else CreateVisualizations(constraints_, n_relevant_constraints_, n_stages_to_construct_+1);
}

void StaticPolyhedronConstraints::CreateVisualizations(const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints, int max_n_stages)
{
    ...
    vts_markers.setScale(ptr_config_->constraints_marker_scale_[0], ptr_config_->constraints_marker_scale_[1], ptr_config_->constraints_marker_scale_[2]);
    ...
    for (int k = 0; k < n_idc; k++)
    {
        ...
        {
            MPC_WARN_STREAM("Index to draw (" << index << ") exceeds polyhedra array size (" << max_n_stages << "). Ignoring this index");
            continue;
        }
        if ((ptr_config_->layer_idx_ == ptr_config_->n_layers_-1) && (ptr_config_->n_layers_ > 1))
        {
            line.setColorInt(k, n_idc, ptr_config_->constraints_marker_alpha_hierarchical_);
            vts_markers.setColorInt(k, n_idc, ptr_config_->constraints_marker_alpha_hierarchical_);
        }
        else
        {
        ...
        ComputeVertices(constraints.block(0,index,3*n_relevant_constraints(index),1));
...
void StaticPolyhedronConstraints::ExportDataJson(const int count_since_start, const int count_total)
{
    PROFILE_AND_LOG_INFO("StaticPolyhedronConstraints::ExportDataJson");
}

// Callback methods
void StaticPolyhedronConstraints::OccGridMapCallBack(nav_msgs::OccupancyGrid::ConstPtr occ_grid_map_ptr)
{
...
void StaticPolyhedronConstraints::getLocalOccupiedCells(nav_msgs::OccupancyGrid::ConstPtr occ_grid_map_ptr, vec_Vec2f path)
{
    ...
    cv::Mat image = cv::Mat(occ_grid_map_ptr->info.height, occ_grid_map_ptr->info.width, CV_8UC1, const_cast<schar*>(&occ_grid_map_ptr->data[0]), (int)occ_grid_map_ptr->info.width);
    ...
    for (size_t k = 0; k < path.size()-1; k++){
        ...
        // convert to gridmap data info. Starting point in data and size for row and column
        int cell_start_x = (min_x - occ_grid_map_ptr->info.origin.position.x) * world_to_image_scalar - img_radius_around_bbox;
        int cell_start_y = (min_y - occ_grid_map_ptr->info.origin.position.y) * world_to_image_scalar - img_radius_around_bbox;
        int n_cols = (max_x - min_x) * world_to_image_scalar + 2*img_radius_around_bbox;
        int n_rows = (max_y - min_y) * world_to_image_scalar + 2*img_radius_around_bbox;
        ...
        double roiImage_origin_x = cell_start_x * image_to_world_scalar + occ_grid_map_ptr->info.origin.position.x;
        double roiImage_origin_y = cell_start_y * image_to_world_scalar + occ_grid_map_ptr->info.origin.position.y;
        ...
        occ_pos_vec_stages_[k]->reserve(size);
        {
            ...
            for (size_t i = 0; i < contours.size(); i++) 
            {
                ...
                for (size_t j = 0; j < contours[i].size(); j++) {
                    // Add the contour point to the vector of points close to the path point
                    double x = roiImage_origin_x + contours[i][j].x*image_to_world_scalar;
                    double y = roiImage_origin_y + contours[i][j].y*image_to_world_scalar;

                    occ_pos_vec_stages_[k]->emplace_back(Eigen::Matrix<double, 2, 1>(x, y));
...
bool StaticPolyhedronConstraints::ComputeConstraints()
{
    ...
    
    {
        ...
        // Depending on the constraints configuration, dilate the path (circular/ellipsoidal decomposition)
        switch (ptr_config_->line_segment_to_stage_)
        {
        case -1:
...

    {
        ...
        // Obtain constraints
        decomp_util_.set_constraints(poly_constraints_, ptr_config_->robot_center_of_mass_to_back_+ptr_config_->safety_margin_);
        ...
        for (int k = 1; k < n_stages_to_construct_+1; k++)
        {
            ...
            {
                MPC_ERROR("StaticPolyhedronConstraints: Amount of relevant constraints (" << n_relevant_constraints_(k) << ") exceeds maximum limit of " << n_constraints_per_stage_ << ". Using first " << n_constraints_per_stage_ << "constraints");
    ...
    if (poly_constraints_.size() > 0)
    {
        ...
        {
            success = success && (int)poly_constraints_[i].A_.rows() >= 4;  // constraints should always consist of the bounding box
        }
    }
    else
    {
    ...
    // Print the polyhedron constraints for all stages
    for (int k = 1; k < n_stages_to_construct_+1; k++)
    {
        MPC_INFO_STREAM("Polyhedron constraints for stage " << k << ":");
        MPC_INFO_STREAM("A: " << poly_constraints_[k-1].A_);
        MPC_INFO_STREAM("b: " << poly_constraints_[k-1].b_);
    }
...
bool StaticPolyhedronConstraints::ComputeLineSegments()
{
    ...
    {
        double x = ptr_solver_->stage_vars_(x_position_, k);
        double y = ptr_solver_->stage_vars_(y_position_, k);
        double yaw = ptr_solver_->stage_vars_(psi_position_, k);

        double error = 0;

        if (k > 0)
        {
            double x_old = ptr_solver_->stage_vars_(x_position_, k-1);
            double y_old = ptr_solver_->stage_vars_(y_position_, k-1);

            if ( (x-x_old == 0) && (y-y_old == 0)){error = double(rand())/double((RAND_MAX))*10e-4;};
        }

        pred_traj_.poses[k].pose.position.x = x + error*cos(yaw);
        pred_traj_.poses[k].pose.position.y = y + error*sin(yaw);
        pred_traj_.poses[k].pose.orientation.z = yaw;  // abusing quaternion z element for yaw rotation about z-axis
    }

    // Construct path elements based on predicted positions of previous run
    // Note: see constructor StaticPolyhedronConstraints() for more details about the convention
    path_.clear();
    ...
    case -1:
        ...
        {
            point << pred_traj_.poses[k].pose.position.x, pred_traj_.poses[k].pose.position.y;
            path_.push_back(point);
        }
        point << pred_traj_.poses[ptr_solver_->n_].pose.position.x+ptr_config_->delta_segment_*cos(pred_traj_.poses[ptr_solver_->n_].pose.orientation.z), pred_traj_.poses[ptr_solver_->n_].pose.position.y+ptr_config_->delta_segment_*sin(pred_traj_.poses[ptr_solver_->n_].pose.orientation.z);
        path_.push_back(point);
        point << pred_traj_.poses[ptr_solver_->n_].pose.position.x+2*ptr_config_->delta_segment_*cos(pred_traj_.poses[ptr_solver_->n_].pose.orientation.z), pred_traj_.poses[ptr_solver_->n_].pose.position.y+2*ptr_config_->delta_segment_*sin(pred_traj_.poses[ptr_solver_->n_].pose.orientation.z);
        path_.push_back(point);
        break;
    ...
    case 0:
        for (int k = 2; k < ptr_solver_->n_+1; k++)
        {
            point << pred_traj_.poses[k].pose.position.x, pred_traj_.poses[k].pose.position.y;
            path_.push_back(point);
            point << pred_traj_.poses[k].pose.position.x+ptr_config_->delta_segment_*cos(pred_traj_.poses[k].pose.orientation.z), pred_traj_.poses[k].pose.position.y+ptr_config_->delta_segment_*sin(pred_traj_.poses[k].pose.orientation.z);
            path_.push_back(point);
        }
        point << pred_traj_.poses[ptr_solver_->n_].pose.position.x+ptr_config_->delta_segment_*cos(pred_traj_.poses[ptr_solver_->n_].pose.orientation.z), pred_traj_.poses[ptr_solver_->n_].pose.position.y+ptr_config_->delta_segment_*sin(pred_traj_.poses[ptr_solver_->n_].pose.orientation.z);
        path_.push_back(point);
    ...
    default:
        MPC_ERROR("StaticPolyhedronConstraints: Unknown value for 'ptr_config_->line_segment_to_stage_'!");
        return false;
        break;
    }
...
// Helper methods
void StaticPolyhedronConstraints::ComputeVertices(const Eigen::VectorXd& constraints)
{
    ...
    for (size_t i = 0; i < n_constraints; i++) {
        for (size_t j = i + 1; j < n_constraints; j++) {
            ...
            det = constraints(3*i) * constraints(3*j+1) - constraints(3*i+1) * constraints(3*j);
            if (std::abs(det) < 1e-9) continue;  // Ignore parallel lines
            Eigen::Vector2d x;
            x << (constraints(3*j+1) * constraints(3*i+2) - constraints(3*i+1) * constraints(3*j+2)) / det,
                 (constraints(3*i) * constraints(3*j+2) - constraints(3*j) * constraints(3*i+2)) / det;

            // Check if the intersection point satisfies all constraints
            satisfies_all_constraints = true;
            for (size_t k = 0; k < n_constraints; k++)
            {
                if (k == i || k == j) continue;  // Skip the constraints used to compute the intersection point
                if (constraints(3*k) * x(0) + constraints(3*k+1) * x(1) > constraints(3*k+2) + FLOAT_TOL)
                {
            ...
            // If the intersection point satisfies all constraints, add it to the list of vertices
    ...
    std::sort(vertices_to_vis_.begin(), vertices_to_vis_.end(), [&centroid](const Eigen::Vector2d& a, const Eigen::Vector2d& b) {return std::atan2(a(1) - centroid(1), a(0) - centroid(0)) < std::atan2(b(1) - centroid(1), b(0) - centroid(0));});
}
...
bool StaticPolyhedronConstraints::GetOccupiedGridCells(const nav_msgs::OccupancyGrid &occ_grid_map)
{
    PROFILE_FUNCTION();
...
int n_points = 0;
    ...
    if (ptr_config_->occ_cell_selection_method_ == 0)
    {
        ...
        n_points = ids.size();
        MPC_INFO_STREAM("StaticPolyhedronConstraints: Amount of occupied cells in grid map: " << n_points);
        ...
        for (int i = 0; i < (int)contours_.size(); i++)
        {
            ...
            {
                x = occ_grid_map.info.origin.position.x + contours_[i][j].x*occ_grid_map.info.resolution;
                y = occ_grid_map.info.origin.position.y + contours_[i][j].y*occ_grid_map.info.resolution;
    ...
    else if (ptr_config_->occ_cell_selection_method_ == 1)
    {
        ...
        {
            MPC_ERROR("StaticPolyhedronConstraints: Determining obstacle contours via grid map and OpenCV failed!");
            exit(1);
        }
        ...
        MPC_INFO_STREAM("StaticPolyhedronConstraints: Amount of occupied obstacle contour cells in grid map: " << n_points);
    ...
    {
        MPC_ERROR("StaticPolyhedronConstraints: Unknown value for 'ptr_config_->occ_cell_selection_method_'!");
        return false;
    }
...
// Helpers
bool StaticPolyhedronConstraints::GetObstacleContours(const nav_msgs::OccupancyGrid &occ_grid_map, std::vector<std::vector<cv::Point>> &contours)
{
    PROFILE_FUNCTION();
    ...
    // Convert occupancy grid manually to OpenCV image directly
    {
        ...
        image = cv::Mat(occ_grid_map.info.height, occ_grid_map.info.width, CV_8UC1, const_cast<schar*>(&occ_grid_map.data[0]), (int)occ_grid_map.info.width);
    }
