...
ModuleHandler::ModuleHandler(ros::NodeHandle &nh, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, DataSaverJson *ptr_data_saver_json)
...
{
    int index = 0;
    for (std::vector<std::string>::iterator iter = ptr_solver->modules_names_.begin(); iter != ptr_solver->modules_names_.end(); iter++)
    {
#ifdef reference_path
        if (*iter == "ReferencePath")
        {
            ...
            modules_pointers_[index].reset(new ReferencePath(modules_names_[index], nh, ptr_config, ptr_solver, ptr_data_saver_json));
            ROS_WARN_STREAM("[Module handler] " << modules_names_[index] << " module loaded");
            index++;
            continue;
        }
#endif
        ...
        if (*iter == "ReferenceTrajectory")
        {
            ...
            modules_pointers_[index].reset(new ReferenceTrajectory(modules_names_[index], nh, ptr_config, ptr_solver, ptr_data_saver_json));
            ROS_WARN_STREAM("[Module handler] " << modules_names_[index] << " module loaded");
            index++;
            continue;
        }
        ...
        if (*iter == "GoalOriented")
        {
            ...
            modules_pointers_[index].reset(new GoalOriented(modules_names_[index], nh, ptr_config, ptr_solver, ptr_data_saver_json));
            ROS_WARN_STREAM("[Module handler] " << modules_names_[index] << " module loaded");
            index++;
            continue;
        }
        ...
        if (*iter == "RobustTightening")
        {
            ...
            modules_pointers_[index].reset(new RobustTightening(modules_names_[index], nh, ptr_config, ptr_solver, ptr_data_saver_json, data_));
            ROS_WARN_STREAM("[Module handler] " << modules_names_[index] << " module loaded");
            index++;
            continue;
        }
        ...
        if (*iter == "StaticPolyhedronConstraints")
        {
            ...
            modules_pointers_[index].reset(new StaticPolyhedronConstraints(modules_names_[index], nh, ptr_config, ptr_solver, ptr_data_saver_json, data_));
            ROS_WARN_STREAM("[Module handler] " << modules_names_[index] << " module loaded");
            index++;
            continue;
        }
#endif
        ROS_ERROR_STREAM("[Module handler] Specified module from the solver isn't found (Not correctly defined in the CmakeList or not added to module_handler.cpp)! name: " << *iter);
    }
}
...
void ModuleHandler::onDataReceivedObjective(std::string name)
{
    ...
    {
        if (module->type_ == ModuleType::OBJECTIVE) {module->OnDataReceived(data_, name);};
    }
}
...
void ModuleHandler::onDataReceivedConstraint(std::string name)
{
    for (auto &module : modules_pointers_)
    {
        if (module->type_ == ModuleType::CONSTRAINT) {module->OnDataReceived(data_, name);};
    }
}
...
void ModuleHandler::setParametersObjective()
{
    for (auto& module : modules_pointers_)
    {
        if (module->type_ == ModuleType::OBJECTIVE){module->SetParameters(data_);};
    }
}
...
void ModuleHandler::setParametersConstraint()
{
    for (auto& module : modules_pointers_)
    {
        if (module->type_ == ModuleType::CONSTRAINT){module->SetParameters(data_);};
    }
}
...
bool ModuleHandler::checkModulesReady()
{
    ...
    for (auto& module : modules_pointers_)
    {
        ...
        if(!module_ready_){ROS_WARN_STREAM("[Module handler] Module " << modules_names_[i] << " not ready for control!");};
        i++;
    }  
...
bool ModuleHandler::checkObjectiveReached()
{
    ...
    if(objective_reached){ROS_WARN_STREAM("[Module handler] Objective reached!");};
    return objective_reached;
}

void ModuleHandler::tightenConstraints(std::string module_name, double c_o, double alpha)
{
    for (auto& module : modules_pointers_)
    {
        if (module->type_ == ModuleType::CONSTRAINT && module->name_ == module_name)
        {
            module->TightenConstraints(c_o, alpha);
        }
    }
}

void ModuleHandler::setConstraintsToRecAndVis(std::string module_name, const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints)
{
    for (auto& module : modules_pointers_)
    {
        if (module->type_ == ModuleType::CONSTRAINT && module->name_ == module_name)
...
{
    std::string s;
    for (std::vector<std::string>::const_iterator i = ptr_solver_->modules_names_.begin(); i != ptr_solver_->modules_names_.end(); ++i)
        s += "_" + *i;
    return s;
}

void ModuleHandler::printRealTimeData()
{
    data_.Print();
}
