...
public:
     ...
     */
    ControllerModule(ModuleType type, std::string name, ros::NodeHandle &nh, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, DataSaverJson* ptr_data_saver_json):
        type_(type),
        name_(name),
        nh_(nh),
...
public:
    ...
    */
     ...
     */
    virtual bool ReadyForControl(const RealTimeData &data) { return true; }; // Default: true
     ...
     */
    virtual bool ObjectiveReached(const RealTimeData &data) { return true; }; // Default: true
     ...
     * @param custom_plan 
     */
    //virtual void SetWarmStart(std::unique_ptr<std::vector<std::vector<double>>> &custom_plan) {};
    ...
    /**
     * @brief Tighten the constraints using c_o and alpha
     * 
     * @note This function is only used if the module is a constraint module
     */
    virtual void TightenConstraints(double c_o, double alpha){};

    /**
     * @brief Set custom constraints via system interface in the constraints module
     * 
     * @note This function is only used if the module is a constraint module
     * 
     ...
     */
    virtual void SetConstraintsToRecAndVis(const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints){};
     ...
     * @param count_since_start Control loop iteration count since start of the current control run
     * @param count_total Control loop iteration count since start of node
     */
    virtual void ExportDataJson(const int count_since_start, const int count_total){};
