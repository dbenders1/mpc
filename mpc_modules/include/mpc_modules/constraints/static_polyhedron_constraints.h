...
// Dont move opencv include above decomp packages, this will give a typedef error!
 ...
 * Constraints are based on a convex decomposition of halfspace constraints given for the current vehicle position with respect to each occupied cell in the occupancy grid.
 * See https://github.com/sikang/DecompUtil for more information.
 */

class StaticPolyhedronConstraints : public ControllerModule
{
public:
    StaticPolyhedronConstraints(std::string name, ros::NodeHandle &nh, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, DataSaverJson *ptr_data_saver_json, RealTimeData &data);
...
protected:
    ...
    void TightenConstraints(double c_o, double alpha) override;
    void SetParameters(const RealTimeData &data) override;
    void SetConstraintsToRecAndVis(const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints) override;
    ...
    void getLocalOccupiedCells(nav_msgs::OccupancyGrid::ConstPtr occ_grid_map_ptr, vec_Vec2f path);
    void CreateVisualizations(const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints, int max_n_stages);
    void ComputeVertices(const Eigen::VectorXd& constraints);

    // Old functions for DECOMP_OLD definition
    bool GetObstacleContours(const nav_msgs::OccupancyGrid &occ_grid_map, std::vector<std::vector<cv::Point>> &contours);
...
private:
    /**
     * @brief Compute the line segments using predicted trajectory from previous solver run
     * 
    ...
    /**
     * @brief Constraint generation function for all steps in the prediction horizon
     * 
     * @return true
     * @return false
     */
    ...
    // We get the solver data as matrix or vector, and need to identify which positions we need
    int x_position_;
    int y_position_;
    int psi_position_;
    int disc0_0_a1_position_;

    // Index of stage we want to start inserting parameters
    int start_index_set_param_;
    ...
    std::vector<LinearConstraint<2>> poly_constraints_; // Static 2D halfspace constraints set in DecompUtil

    // Constraints variables
    // NOTE: Eigen stores in column-major order, so stack 3*n_constraints_per_stage_ values per column, 1+N columns in total to first iterate through values of constraints per stage, then through stages
    bool first_constraints_constructed_ = false;
    int n_stages_to_construct_, n_constraints_per_stage_;
    Eigen::MatrixXd constraints_; // 3*n_constraints_per_stage_ x 1+N (stage 0 comes from the previous run and is not used in this run)
    Eigen::VectorXi n_relevant_constraints_; // Number of relevant constraints per stage
    ...
    // NOTE: the size of these constraints depends on the type of MPC and what the system interface indicates, so need to be able to resize during runtime
