...
// Forward decleration of classes. This can be done if we don't exactly use the class itself in this file.
// Therefore we can exclude a header include completely or move it to the cpp file.
...
public:
    ...
    /**
     * @brief This is the main module loader. The input is received from the solver package which contains the names
     * of the modules that we are going to use.
     * 
     * @param nh 
     * @param config 
     * @param vehicle 
     ...
     */
    ModuleHandler(ros::NodeHandle &nh, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, DataSaverJson *ptr_data_saver_json);
    ...
    /**
     * @brief Tighten the constraints using c_o and alpha in constraints module with name module_name
    */
    void tightenConstraints(std::string module_name, double c_o, double alpha) override;

    /**
     * @brief Set custom constraints via system interface in the constraints module with name module_name
     * 
     * @param module_name name of the module
     * @param constraints constraints to set
     * @param n_relevant_constraints number of relevant constraints
    */
    void setConstraintsToRecAndVis(std::string module_name, const Eigen::MatrixXd &constraints, const Eigen::VectorXi &n_relevant_constraints) override;
