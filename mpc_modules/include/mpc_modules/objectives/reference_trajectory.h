...
public:
    ReferenceTrajectory(std::string name, ros::NodeHandle &nh, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, DataSaverJson* ptr_data_saver_json);
    ...
    void SetWarmStart(std::unique_ptr<std::vector<std::vector<double>>> &custom_plan);
...
private:
    ...
    // Counter of amount of times the controller has run => to shift the reference trajectory forward in time
    int n_shift_;
    ...
    std::unique_ptr<ROSMarkerPublisher> reference_trajectory_marker_pub_, reference_trajectory_ground_marker_pub_;
};
