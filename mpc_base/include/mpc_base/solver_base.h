...
public:
    ...
    // Horizon length
    const int n_;
    const int nbar_; // ForcesPro needs one more stage to implement the usual 0-N MPC structure

    // Stage variables
    const int ninit_; // Number of variables that are equality-constrained in stage 0 (most of the times equal to nx)
    const int nu_;
    const int nx_;
    const int n_stage_vars_;
    std::vector<std::string> stage_vars_names_;
    // At the start of each loop, after actuating, last_updated_stage_vars_ is loaded into the stage_vars_ structure,
    // this to start each loop with the closest measured state.
    // So this structure is the output from the solver but then again also the input for the next step
    ...
    SolverBase(double dt,
        ...
        constants_(Eigen::Map<Eigen::VectorXd>(constants.data(), constants.size())),
        npar_(n_par),
    ...
    // Templated functions cannot be virtual, so need to define the type of constant beforehand and perform function overloading
    // (compiler needs to know what the type is during, but type in case of virtual is determined only during runtime)
    ...
    virtual Eigen::VectorXd getConstantVectorDouble(std::string constant_name)=0;
    virtual Eigen::MatrixXd getConstantMatrixDouble(std::string constant_name)=0;

    virtual std::vector<int> returnDataPositionsState(std::vector<std::string> find_names)=0;

	virtual std::vector<int> returnDataPositionsObjective(std::vector<std::string> find_names)=0;

	virtual std::vector<int> returnDataPositionsConstrain(std::vector<std::string> find_names)=0;
