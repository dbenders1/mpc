...
#define MPC_PRINT_CONFIG(type, msg) \
	...
	{ \
		...
		if (ptr_config_->n_layers_ > 1) msg_start = msg_start + " " + std::to_string(ptr_config_->layer_idx_); \
		msg_start.append("] "); \
...
#define MPC_INFO_SOLVER_LAYER(layer_idx, msg) \
	...
	{ \
		std::string type = "info"; \
		std::string msg_start = MPC_COMMON_MSG_START + " " + std::to_string(layer_idx) + "] "; \
		MPC_PRINT_ROS(type, msg_start, msg) \
	} while(0);
...
#define MPC_ASSERT(Expr, Msg) \
    __MPC_ASSERT(#Expr, Expr, __FILE__, __LINE__, Msg)
#define MPC_HOOK std::cout << __FILE__ << " Line " << __LINE__ << std::endl;
#define MPC_HOOK_MSG(MSG) std::cout << __FILE__ << " Line " << __LINE__ << " [Message: " << MSG << "]" << std::endl;
#else
#define MPC_ASSERT(Expr, Msg)
#define MPC_HOOK
#define MPC_HOOK_MSG(MSG)
#endif
inline void __MPC_ASSERT(const char *expr_str, bool expr, const char *file, int line, const char *msg)
{
    if (!expr)
    {
