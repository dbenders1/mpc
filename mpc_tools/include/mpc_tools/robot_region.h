...
/**
 * @brief Models a robot region shaped as a disc. Positions are derived based on offsets and the vehicle position.
 */
struct Disc
{
...
public:
     ...
     * @param radius Radius
     */
    Disc(int id, const Eigen::Vector2d &pos, const double orientation, const double offset, const double radius)
...
public:
     ...
     */
    RobotRegionTemplate(int n_discs, const std::vector<double> &offsets, double radius);
    RobotRegionTemplate(){};
...
public:
    ...
    RobotRegion(const Eigen::Vector2d &pos, const double orientation, int n_discs, double width, double length, double center_offset); // int n_discs, predictive_configuration *config); // SolverInterface *solver_interface, predictive_configuration *config);
    /** @brief Use a template to create a vehicle region at the given position / orientation */
    RobotRegion(const Eigen::Vector2d &pos, const double orientation, const RobotRegionTemplate &vehicle_template); // int n_discs, predictive_configuration *config); // SolverInterface *solver_interface, predictive_configuration *config);
