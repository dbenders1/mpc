...
public:
    ...
    /* Main public methods */
    /**
     * @brief Add static data to the json object. Supports STL containers (see https://github.com/nlohmann/json#conversion-from-stl-containers)
     * 
     * @tparam T 
     * @param data_name 
     * @param data_value 
    ...
    /**
     * @brief Add runtime data to the json object. Supports STL containers (see https://github.com/nlohmann/json#conversion-from-stl-containers)
     * 
     * @tparam T 
     ...
     */
    template <typename T>
    void AddRuntimeData(const std::string &&data_name, const int &count_since_start, const int &count_total, const T &data_value)
    {
        // Create json object for runtime data element
        json data;
        data["number"] = runtime_data_count_;
        data["time"] = std::chrono::duration<double>(std::chrono::system_clock::now().time_since_epoch()).count();
        data["name"] = data_name;
...
private:
    ...
    /**
     * @brief Save data in JSON format to specified file in mpc_tools/recorded_data
     * 
     */
    void StoreInFile();
    ...
    /**
     * @brief Determine whether to put a zero in front of month, day, hour or minutes
     * 
     * @param value 
     * @return std::string 
     */
