...
{
    // If we exceed the max size, allocate 1.5 times the space and give a warning
    if (marker.id > max_size_ - 1)
    {
        ...
        ros_markers_.reserve(max_size_);
    }
...
{
    // Create a line
    // For vector of std::unique_ptr preferably use push_back (not emplace_back), see https://stackoverflow.com/a/15784813
    ros_markers_.push_back(std::unique_ptr<ROSMarker>(new ROSLine(this, frame_id_)));

    // Return a pointer
    return *((ROSLine *)(ros_markers_.back().get()));
}

ROSPointMarker &ROSMarkerPublisher::getNewPointMarker(const std::string &marker_type)
{
    // Create a point marker
    ros_markers_.push_back(std::unique_ptr<ROSMarker>(new ROSPointMarker(this, frame_id_, marker_type)));

    // Return a pointer
    return *((ROSPointMarker *)(ros_markers_.back().get()));
}

ROSMultiplePointMarker &ROSMarkerPublisher::getNewMultiplePointMarker(const std::string &marker_type)
{
    // Create a point marker
    ros_markers_.push_back(std::unique_ptr<ROSMultiplePointMarker>(new ROSMultiplePointMarker(this, frame_id_, marker_type)));

    // Return a pointer
    return *((ROSMultiplePointMarker *)(ros_markers_.back().get()));
}
...
inline void ROSMarker::setLifetime(double lifetime)
{

    marker_.lifetime = ros::Duration(lifetime);
}

inline void ROSMarker::setActionDelete()
{
...
void ROSLine::addCircularLine(const Eigen::Vector2d &center, double radius, double z, double n_points)
{
    // Create n_points points in a circle centered at center with specific radius at height z
    std::vector<geometry_msgs::Point> points(n_points);
    for (int i = 0; i < n_points; i++)
    {
        points[i].x = center(0) + radius * std::cos(2 * M_PI * i / n_points);
        points[i].y = center(1) + radius * std::sin(2 * M_PI * i / n_points);
        points[i].z = z;
    }
    ...
    for (int i = 0; i < n_points; i++)
    {
        if (i == n_points - 1)
        {
            marker_.points.push_back(points[i]);
            marker_.points.push_back(points[0]);
        }
        else
        {
            marker_.points.push_back(points[i]);
            marker_.points.push_back(points[i+1]);
        }
    }

    // Add marker to marker publisher
    ros_publisher_->add(marker_);
...
inline void ROSPointMarker::setZ(double z)
{
...
inline void ROSMultiplePointMarker::addPointMarker(const geometry_msgs::Point &p1)
{
    marker_.points.push_back(p1);
}
...
inline void ROSTextMarker::setZ(double z)
{
    marker_.pose.position.z = z;
}

inline void ROSTextMarker::setText(const std::string &text)
{
    marker_.text = text;
}
...
inline void ROSTextMarker::setScale(double z)
{
