...
DataSaverJson::DataSaverJson()
{
    ...
    start_date_time_stamp_ = 
        ParseYear(local_time.tm_year) + "-" +
...
void DataSaverJson::set_layer_idx(int layer_idx)
{
...
void DataSaverJson::StoreInFile()
{
    ...
    if (boost::filesystem::create_directories(dir_path))
        std::cout << "Data saver JSON: Creating directory path " << dir_path << std::endl;
    
    // Determine file path based on available data
    std::string file_name = start_date_time_stamp_ + "_" + exp_name_;
    if (layer_idx_ != -1)
        file_name += "_" + std::to_string(layer_idx_);
    file_name += ".json";
