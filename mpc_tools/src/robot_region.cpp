...
Eigen::Vector2d Disc::TranslateToVehicleCenter(const double orientation) const
{
    return Eigen::Vector2d(
        x_ - offset_ * std::cos(orientation),
...
RobotRegionTemplate::RobotRegionTemplate(int n_discs, const std::vector<double> &offsets, double radius)
{
    offsets_ = offsets;
...
RobotRegion::RobotRegion(const Eigen::Vector2d &pos, const double orientation, int n_discs, double width, double length, double center_offset)
{
...
RobotRegion::RobotRegion(const Eigen::Vector2d &pos, const double orientation, const RobotRegionTemplate &vehicle_template)
{
    // Copy the template
...
void RobotRegion::SetPosition(const Eigen::Vector2d &new_pos, const double orientation)
{
    for (auto &disc : discs_)
