...
Controller::Controller(ros::NodeHandle& nh, ConfigurationMPC* ptr_config, SolverBase* ptr_solver, InterfaceBase* ptr_interface, ModuleHandlerBase* ptr_module_handler, DataSaverJson* ptr_data_saver_json)
    :nh_(nh),
     ...
     timer_(nh.createTimer(ros::Duration(ptr_solver->dt_), std::bind(&Controller::runNode, this,  std::placeholders::_1)))
{
  MPC_WARN_ALWAYS("Initializing controller");

  // Initialize profiling
  Helpers::Instrumentor::Get().BeginSession("MPC", ros::this_node::getName(), std::string("mpc_profiler_") + std::to_string(ptr_config_->layer_idx_) + std::string(".json"));
  ...
  {
    pub_computation_times_ = nh.advertise<sensor_msgs::Temperature>("/mpc/computation_times/"+ std::to_string(ptr_config_->layer_idx_), 1);
  } else
  {
    pub_computation_times_ = nh.advertise<sensor_msgs::Temperature>("/mpc/computation_times", 1);
  }
  computation_time_msg_.header.stamp = ros::Time::now();
  ...
  // Stop the control loop timer by default, so its start can be regulated in the system interface
  timer_.stop();
...
void Controller::setInterface(InterfaceBase* ptr_interface)
{
  ...
  if (ptr_config_->record_experiment_json_)
  {
    ...
    MPC_WARN_STREAM("Recording json file with name: " << ptr_config_->recording_name_json_);
...
void Controller::runNode(const ros::TimerEvent& event)
{
  controlLoop();
}
...
void Controller::controlLoopStart()
{
  ...
  mpc_header_.t_start = ros::Time::now();
...
{
  PROFILE_FUNCTION();
  control_loop_profiling_string_ = "controlLoop (count total: "+std::to_string(timer_.getCountTotal())+")";

  {
    PROFILE_SCOPE(control_loop_profiling_string_.c_str());
    controlLoopStart();

    // Actuate at the start of the control loop is we expect one control loop of delay
    if (ptr_config_->synchronized_actuation_){ptr_interface_->Actuate();};

    // System interface-dependant computations at start control loop
    // Need to call it at this place, because module data might need to be updated in this function before the checkModulesReady() call
    ...
    // The control loop runs if we are planning and all data was received else we return from the loop
    if (!ptr_config_->plan_ || !state_received_ || !modules_are_ready_)
    {
      PROFILE_SCOPE("No control");
      if (!ptr_config_->plan_) {PROFILE_AND_LOG_WARN("Planning disabled");}
      if (!state_received_) {PROFILE_AND_LOG_WARN("No state received");}
      if (!modules_are_ready_) {PROFILE_AND_LOG_WARN("Modules not ready");}
      ptr_interface_->DeployEmergencyStrategy();
      controlLoopEndNotCompleted();
      return;
    }

    // Do not run the control loop in synchronous mode with simple_sim and lowest layer MPC before having received a state update
    if (ptr_config_->sync_simple_sim_ && ptr_config_->layer_idx_ == 0 && !updated_state_received_)
    {
      PROFILE_SCOPE("No control simplesim");
      MPC_WARN("Need to sync with simplesim, but no state received from simplesim yet");
      controlLoopEndNotCompleted();
      return;
    }
    else{updated_state_received_ = false;};

    // Everything OK with the checks 
    controller_status_ = ControllerStatus::SUCCESS;

    // In the first full run, we have to load in the state in all stages since stage_vars_ is still empty
    if (first_loop_)
    {
    ...
    // Note: module updates should come before shiftHorizonInMatrix(), because information is lost by this step (initial state of previous run), which is used in some modules
    {
    ...
    if (ptr_config_->debug_data_){ptr_module_handler_->printRealTimeData();};

    // Load warm start (using time shift, by default 1), initial state and parameters
    if (!first_loop_)
    {
    ...
    if (ptr_config_->record_experiment_json_){ptr_module_handler_->exportDataJson(timer_.getCountSinceStart(), timer_.getCountTotal());};

    // OPTIMIZATION: input the data into the solver structs and solve the optimisation
    {
      ...
      {
        ptr_solver_->stage_vars_.topRightCorner(4, ptr_solver_->n_) = ptr_solver_->par_objectives_.topRightCorner(4, ptr_solver_->n_);
        ptr_solver_->stage_vars_.bottomRightCorner(ptr_solver_->nx_, ptr_solver_->n_) = ptr_solver_->par_objectives_.bottomRightCorner(ptr_solver_->nx_, ptr_solver_->n_);
      }

      exit_code_ = ptr_solver_->runSolverStep();
      int i = 0;
      while ((exit_code_ == 0 || exit_code_ == -6 || exit_code_ == -7 ) && i < 3)
      {
        MPC_WARN_STREAM("[Solver] exit_code: " << exit_code_ << "    iter: " << i);
        
        Eigen::MatrixXd rand = 1 * Eigen::MatrixXd::Random(ptr_solver_->nu_ + ptr_solver_->nx_, ptr_solver_->n_-1) - 0.5*Eigen::MatrixXd::Ones(ptr_solver_->nu_ + ptr_solver_->nx_, ptr_solver_->n_-1);
        ptr_solver_->stage_vars_.topRightCorner(ptr_solver_->nu_ + ptr_solver_->nx_, ptr_solver_->n_-1) += rand;
    ...
    if (exit_code_ == 1)
    {
      ...
      if (timer_.getCountSinceReset() != 0 && exit_code_ != previous_exit_code_) PROFILE_AND_LOG_WARN("Recovered from infeasibility");
...
bool Controller::timerIsRunning()
{
  return timer_.isRunning();
}

int Controller::getCountSinceReset()
{
  return timer_.getCountSinceReset();
}

int Controller::getCountTotal()
{
  return timer_.getCountTotal();
}
...
void Controller::resetTimerFlag()
{
  timer_.resetFlag();
}
...
void Controller::OnStateReceived()
{
  ...
  bool state_jumped = ((Eigen::Vector2d(current_x, current_y) - Eigen::Vector2d(prev_x_, prev_y_)).norm() > 5.0);
  prev_x_ = current_x;
  prev_y_ = current_y;
  ...
  if (state_received_ && !state_jumped){return;};

  if (!first_loop_){MPC_WARN("Jump in state detected!");};
...
void Controller::OnReset()
{
  ...
  // Stop the timer
  if (ptr_config_->sync_mode_){timer_.stop();};
  ...
  if (ptr_config_->sync_mode_){timer_.start();};
