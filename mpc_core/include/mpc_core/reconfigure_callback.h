...
//    Interface* system_interface_;
    ...
    ReconfigureCallback(SolverBase* ptr_solver, ros::NodeHandle nh_predictive, void (*func)(TConfig&, SolverBase*)):
        ptr_solver_(ptr_solver),
        nh_predictive_(nh_predictive),
        reconfigure_server_(reconfig_mutex_, nh_predictive_),
        func_(func)
    {
        // reconfigure_server_ = new dynamic_reconfigure::Server<TConfig>(reconfig_mutex_, nh_predictive_);
        func_ = func;
        reconfigure_server_.setCallback(boost::bind(&ReconfigureCallback<TConfig>::callbackFunction, this, _1, _2));
        // reconfigure_server_->setCallback(boost::bind(&ReconfigureCallback<TConfig>::callbackFunction, this, _1, _2));
    ...
    void callbackFunction(TConfig& ptr_callback_config, uint32_t level)
    {
        ...
        ptr_solver_->print_solver_param_ = ptr_callback_config.print_solver_parameters;
