...
HovergamesSimpleSimInterface::HovergamesSimpleSimInterface(ros::NodeHandle& nh, ControllerBase *ptr_controller, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, ModuleHandlerBase *ptr_module_handler, RobotRegion *ptr_robot_region, DataSaverJson *ptr_data_saver_json, std::mutex *mutex)
    : HovergamesDroneInterface(nh, ptr_controller, ptr_config, ptr_solver, ptr_module_handler, ptr_robot_region, ptr_data_saver_json, mutex)
{
    ...
    if (ptr_config_->layer_idx_ == 0)
    {
        ...
        command_pub_0_ = nh_.advertise<simple_sim::DroneHovergamesControl>(ptr_config_->control_command_topic_, 1);
    }
    /*********************************************************/
...
void HovergamesSimpleSimInterface::OnStateCallback(const nav_msgs::Odometry &msg)
{
    PROFILE_FUNCTION();
    ...
    if (!first_state_received_)
    {
        ...
        if (!first_ref_received_ && ptr_config_->n_layers_ > 1 && ptr_config_->layer_idx_ == 0)
        {
            ...
            // Fill in reference trajectory and constraints with hover values at current state
            SetReferenceTrajectoryHover();
            SetConstraintsHover();
        }

        if (ptr_config_->layer_idx_ == 0){ptr_controller_->startTimer();};
    }
}
...
void HovergamesSimpleSimInterface::OnDeployObjectiveReachedStrategy()
{
    ...
    // No steps necessary for the objective reached strategy in the simple sim interface
}
