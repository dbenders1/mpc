...
HovergamesPX4Interface::HovergamesPX4Interface(ros::NodeHandle& nh, ControllerBase *ptr_controller, ConfigurationMPC *ptr_config, SolverBase *ptr_solver, ModuleHandlerBase *ptr_module_handler, RobotRegion *ptr_robot_region, DataSaverJson *ptr_data_saver_json, std::mutex *mutex)
    : HovergamesDroneInterface(nh, ptr_controller, ptr_config, ptr_solver, ptr_module_handler, ptr_robot_region, ptr_data_saver_json, mutex)
{
    MPC_WARN_ALWAYS("Initializing hovergames px4 interface");

    // Subscriber for the hover thrust estimate
    hover_thrust_sub_ = nh.subscribe("/mavros/hover_thrust_estimate", 1, &HovergamesPX4Interface::hoverThrustEstimateCallback, this);
    battery_voltage_sub_ = nh.subscribe("/mavros/battery", 1, &HovergamesPX4Interface::batteryVoltageCallback, this);
    ...
    att_target_msg_.type_mask = 7; //binary: 0000 0111 => ignore everything except attitude and thrust setpoints
    ...
    pos_target_msg_.type_mask = 2552; // Binary for which message attributes not to use
    ...
    {
        time_ref_sub_ = nh.subscribe("/mavros/time_reference", 1, &HovergamesPX4Interface::timeReferenceCallback, this);
        time_ref_pub_ = nh.advertise<sensor_msgs::TimeReference>(ptr_config_->time_reference_rec_topic_, 1);
    ...
    if (ptr_config_->layer_idx_ == 0)
    {
        ...
        enable_control_server_ = nh.advertiseService("/px4_ext_cont_enable", &HovergamesPX4Interface::EnableControlCallback, this);
        disable_control_server_ = nh.advertiseService("/px4_ext_cont_disable", &HovergamesPX4Interface::DisableControlCallback, this);
        
        // Subscribers for sensor data
        imu_sub_ = nh.subscribe("/mavros/imu/data", 1, &HovergamesPX4Interface::imuCallback, this);

        // Publishers for vehicle commands
        attitude_pub_ = nh.advertise<mavros_msgs::AttitudeTarget>(ptr_config_->control_command_topic_, 1);
        position_pub_ = nh.advertise<mavros_msgs::PositionTarget>("/mavros/setpoint_raw/local", 1);

        // Sending out solver information
        acceleration_pub_ = nh.advertise<sensor_msgs::Imu>("/solver_info/acceleration_setpoint", 1);
        position_guess_pub_ = nh.advertise<nav_msgs::Odometry>("/solver_info/position_guess", 1);
    }
    /*********************************************************/
...
void HovergamesPX4Interface::OnStateCallback(const nav_msgs::Odometry &msg)
{
    ...
    Eigen::Quaterniond q = Eigen::Quaterniond(q_tf.w(), q_tf.x(), q_tf.y(), q_tf.z());
...
void HovergamesPX4Interface::OnComputeActuation()
{
    ...
    position_guess_msg_.pose.pose.position.x = ptr_solver_->stage_vars_(x_idx_,0);
    position_guess_msg_.pose.pose.position.y = ptr_solver_->stage_vars_(y_idx_,0);
    position_guess_msg_.pose.pose.position.z = ptr_solver_->stage_vars_(z_idx_,0);
    ...
    position_guess_msg_.twist.twist.linear.x = ptr_solver_->stage_vars_(vx_idx_,0);
    position_guess_msg_.twist.twist.linear.y = ptr_solver_->stage_vars_(vy_idx_,0);
    position_guess_msg_.twist.twist.linear.z = ptr_solver_->stage_vars_(vz_idx_,0);
}
...
void HovergamesPX4Interface::OnActuate()
{
    ...
    // Rotate unit vector in z direction to the body frame to know components of force in world frame
    ...
    // We need to subtract the gravitational force to get the acceleration setpoint
    ...
    double collective_thrust = thrust_scaling_ * (acc_vector_world(2) * (hover_thrust_ / g)) + hover_thrust_;
...
bool HovergamesPX4Interface::EnableControlCallback(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
{
    PROFILE_AND_LOG_INFO("EnableControlCallback: enabling control loop by starting timer");

    // Start control loop when receiving the first state and check if already has reference trajectory and constraints
    if (!first_ref_received_ && ptr_config_->n_layers_ > 1 && ptr_config_->layer_idx_ == 0)
    {
        start_stage_vars_ = ptr_solver_->last_updated_stage_vars_;

        // Fill in refeence trajectory and constraints with hover values at current state
...
bool HovergamesPX4Interface::DisableControlCallback(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
{
    PROFILE_AND_LOG_INFO("DisableControlCallback: disabling control Loop by resetting solver and stopping timer");
...
void HovergamesPX4Interface::batteryVoltageCallback(const sensor_msgs::BatteryState& msg)
{
    PROFILE_FUNCTION();
...
void HovergamesPX4Interface::hoverThrustEstimateCallback(const mavros_msgs::HoverThrustEstimate& msg)
{
    PROFILE_FUNCTION();
...
void HovergamesPX4Interface::timeReferenceCallback(const sensor_msgs::TimeReference& msg)
{
    // First store current time stamp to reduce the delay of receiving the message as much as possible
