...
integrator_options['stepsize'] = 0.05       # Timestep of the integrator - should equal TMPC stepsize (default: 0.05)
integrator_options['steps'] = 10            # beta: integration is done for a total amount of time equal to stepsize*steps - should equal TMPC ratio (default: 10)
...
constants.add_constant("use_tightened_obstacle_constraints", np.array(int(model_options['use_tightened_obstacle_constraints'])))
...
constants.add_constant("n_states_to_constrain", np.array(int(N_total - N + 1))) # Number of states to constrain includes the initial state constraint of the optimization problem with horizon N
# ====================================================================
...
if model_options['use_tightened_system_constraints']:
    ...
    if model_options['use_slack']:
        nvar = nvar - 1
    modules.add_module(control_modules.TightenedSystemConstraints(params, nvar, offline_comp), tightened_system_stages)
...
if model_options["use_terminal_steady_state_constraint"]:
    ...
    modules.add_module(control_modules.TerminalConstraintsSteadyState(model_options['use_slack']), terminal_constraint_stages)
# ====================================================================
