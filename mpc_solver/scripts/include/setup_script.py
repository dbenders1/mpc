...
from generation_scripts import (generate_cmake,
                                generate_dynamic_reconfigure_files,
                                generate_files, generate_solver)


def create_solver(solver_names, system, dir_path, build_solver, use_floating, floating_platform):
    module_setting = []
    ...
    for name in solver_names:
        ...
        if build_solver:
            generate_solver.create_solver(name, system, dir_path, module_setting[k], use_floating, floating_platform)
...
def check_config(system, build_solver,build_solver_interface,number_solvers,setting_files):
# Check if the config file was correct format
    if not isinstance(build_solver, bool) or not isinstance(build_solver_interface, bool):
        print("[ERROR]: build_solver and build_solver_interface should be true or false statement! \n")
        exit()
    ...
    try:
        ...
        print("[ERROR]: system should be a string containing one of the available systems! \n")
        exit()
    ...
    try:
        ...
        print("[ERROR]: something wrong in the configuration.yaml file or in the system directory!\n")
        exit()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--config", nargs="+", help="One or more configuration files")
    parser.add_argument("-s", "--system", required=True, help="System argument")
    parser.add_argument("-f", "--floating_license_platform", required=False, help="Floating license platform argument")
    parser.add_argument("-b", "--build_solver", required=True, help="Build solver argument")
    ...
    print(build_solver, build_solver_interface, number_solvers, setting_files, use_floating, floating_platform)
    
    solver_names = check_config(system, build_solver,build_solver_interface,number_solvers,setting_files)
    ...
    module_setting = create_solver(solver_names, system, dir_package, build_solver, use_floating, floating_platform)
    ...
    if (build_solver_interface):
        ...
        generate_files.generate_cpp_files(solver_names, module_setting, system, dir_package)
        generate_cmake.generate_cmake_files(solver_names, module_setting, system, dir_package)
        generate_dynamic_reconfigure_files.generate_dynamic_reconfigure_files(solver_names, module_setting, system, dir_package)
