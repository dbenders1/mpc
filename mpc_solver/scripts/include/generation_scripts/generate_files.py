...
def generate_cpp_files(Names, module_settings, system, pkg_path):
    ...
    for name in Names:
        ...
        ## Global variables
        DT = module_settings[index].integrator_options['steps'] * module_settings[index].integrator_options['stepsize']
        ...
        for i in range(len(module_settings[index].model.inputs)):
            inputs_names = inputs_names + ",\"" + module_settings[index].model.inputs[i] + "\""
        inputs_names = inputs_names[1:]

        # States
        # States from sensor
        states_from_sensor_names = ""
        i = 0
        for idx, state in enumerate(module_settings[index].model.states):
            if module_settings[index].model.states_from_sensor[idx]:
                i = i + 1
                states_from_sensor_names = states_from_sensor_names + ",\"" + state + "\""
        states_from_sensor_names = states_from_sensor_names[1:]
        # All states
        states_names = ""
        for i in range(len(module_settings[index].model.states)):
            states_names = states_names + ",\"" + module_settings[index].model.states[i] + "\""
        ...
        for constant_dict in module_settings[index].constants.constants:
            ...
            constants_start_idc_str = constants_start_idc_str + "," + str(constant_dict['start_idx'])
        ...
        n_par_objectives = module_settings[index].params.get_number_of_objectives_parameters()[0]
        par_objectives_names = ""
        for objective_name in module_settings[index].params.params[0]['par_objectives_names']:
            par_objectives_names = par_objectives_names + ",\"" + objective_name + "\""
        par_objectives_names = par_objectives_names[1:]

        # Weights parameters
        n_par_weights = module_settings[index].params.get_number_of_weights_parameters()[0]
        par_weights_names = ""
        ...
        # Constraints
        n_par_constraints = module_settings[index].params.get_number_of_constraints_parameters()[0]
        par_constraints_names = ""
        for constraint_name in module_settings[index].params.params[0]['par_constraints_names']:
            par_constraints_names = par_constraints_names + ",\"" + constraint_name + "\"" 
        par_constraints_names = par_constraints_names[1:]

        # All parameters
        pars_names = par_objectives_names + "," + par_weights_names + "," + par_constraints_names
        ...
        h_file_solver.write("// This file is automatically generated \n" +
            "// Time of creation:" + dt_string + "\n" +
            "#ifndef MPC_SOLVER_" + name.upper() +"SOLVER_INTERFACE_H \n" +
            "#define MPC_SOLVER_" + name.upper() +"SOLVER_INTERFACE_H \n\n" +
            "#include <memory> \n\n" +
            "#include <mpc_base/solver_base.h> \n\n" +
            "namespace mpc_solver{ \n\n" +
            "class Solver"+str(index)+" \n" +
            "{\n" +
            "public: \n" +
            "\t" + "SolverBase* solver_" + name + "; \n\n" +
            "\t" + "Solver"+str(index)+"();\n\n" +
            "}; \n" +
            "}/* namespace mpc_solver*/ \n\n" +
            "#endif"        
        ...
        # Create the solver specific header file constructor
        cpp_file_solver.write("// This file is automatically generated \n" +
            "// Time of creation:" + dt_string + "\n" +
            "#include <mpc_solver/" + system +"/" + name +"solver.h> \n\n" + 
            "#include <mpc_solver/core/solver.h> \n\n" +
            "#include <mpc_solver/" + system +"/" + name +"FORCESNLPsolver/include/" + name +"FORCESNLPsolver.h> \n" + 
            "#include <mpc_solver/" + system +"/" + name +"FORCESNLPsolver/include/" + name +"FORCESNLPsolver_memory.h> \n\n" +
            "extern \"C\" \n" +
            "{	\n" +
            "\t#include <mpc_solver/" + system +"/" + name +"FORCESNLPsolver/" + name +"FORCESNLPsolver_interface.c> \n" + 
            "\t#include <mpc_solver/" + system +"/" + name +"FORCESNLPsolver/" + name +"FORCESNLPsolver_model.c> \n" +
            "} \n\n"
            "namespace mpc_solver{ \n\n" +
            "template class Solver<" + name +"FORCESNLPsolver_params, " + name +"FORCESNLPsolver_output, \n" +
            "\t" + name +"FORCESNLPsolver_info, " + name +"FORCESNLPsolver_mem, " +
            name +"FORCESNLPsolver_extfunc, FILE, solver_int32_default>; \n\n" +
            "typedef Solver<" + name +"FORCESNLPsolver_params, " + name +"FORCESNLPsolver_output, " + name +"FORCESNLPsolver_info, \n" +
            "\t" + name +"FORCESNLPsolver_mem, " + name +"FORCESNLPsolver_extfunc, FILE, solver_int32_default> TSolver" + str(index) + ";\n\n" +
            "Solver"+str(index)+"::Solver"+str(index)+"(): " +
            "solver_" + name + "(new TSolver"+str(index)+ "("+str(DT)+", "+str(N)+", "+str(NINIT)+",\n"+
            "\t"+str(NU)+", "+str(NX)+",\n"+
            "\t{"+inputs_names+","+states_names+"},\n"+
            "\t{"+constants_names+"},\n"+
            "\t{"+constants_ndims_str+"},\n"+
            "\t{"+constants_sizes_str+"},\n"+
            "\t{"+constants_start_idc_str+"},\n"+
            ...
            "\t{"+module_names+"},\n"+
            "\t"+str(index)+",\n"+
            "\t&"+name+"FORCESNLPsolver_external_mem,\n"+
	        "\t&"+name+"FORCESNLPsolver_get_mem_size,\n"+
	        "\t&"+name+"FORCESNLPsolver_adtool2forces,\n"+
	        "\t&"+name+"FORCESNLPsolver_solve))\n"+
            "{ };\n"+
            "}/*namespace mpc_solver*/\n\n"        
        )
        ...
        solver_classes = solver_classes + \
            "\tSolver" + str(index) + " solver_" + name + "; \n"
        header_files = header_files + \
            "#include <mpc_solver/" + system +"/" + name +"solver.h> \n"
        
        if index == 0:
            solver_if_statement = solver_if_statement + \
                "\tif (solver_number == "+str(index)+") \n" + \
                "\t{\n" +\
                "\t\tSolver"+str(index)+" solver; \n"+ \
                "\t\tptr_solver = std::unique_ptr<SolverBase>(std::move(solver.solver_"+name+")); \n" +\
                "\t} \n"
        else:
            solver_if_statement = solver_if_statement + \
                "\telse if (solver_number == "+str(index)+") \n" + \
                "\t{\n" +\
                "\t\tSolver"+str(index)+" solver; \n"+ \
                "\t\tptr_solver = std::unique_ptr<SolverBase>(std::move(solver.solver_"+name+")); \n" +\
                "\t} \n"
        ...
        solvers_emplace = solvers_emplace + \
            "\tvector_ptr_solvers.emplace_back();\n" + \
            "\tvector_ptr_solvers.back().reset(std::move(solver"+str(index)+".solver_"+name+")); \n"

        index = index + 1

    
    # Write to the main interface header file
    h_file_interface.write("// This file is automatically generated \n" +
        "// Time of creation:" + dt_string + "\n" +
        "#ifndef MPC_SOLVER_" + system.upper() +"_SOLVER_INTERFACE_H \n" +
        "#define MPC_SOLVER_" + system.upper() +"_SOLVER_INTERFACE_H \n\n" +
        "#include <memory> \n\n" +
        "#include <string> \n\n" +
        "#include <vector> \n\n" +
        "#include <mpc_base/solver_base.h> \n\n" +
    ...
    cpp_file_interface.write("// This file is automatically generated \n" +
        "// Time of creation:" + dt_string + "\n" +
        "#include <mpc_solver/" + system +"/"+ system + "_solver_interface.h> \n\n" +
        header_files +
