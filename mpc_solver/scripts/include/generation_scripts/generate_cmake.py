...
def generate_cmake_files(solver_names, module_settings, system, pkg_path):
    ...
    for name in solver_names:
        solver_libs = solver_libs + "\t${PROJECT_SOURCE_DIR}/include/mpc_solver/"+ system +"/"+ name +"FORCESNLPsolver/lib/lib"+ name +"FORCESNLPsolver.so \n"
        sources = sources + "\tsrc/"+ system +"/"+ name +"solver.cpp \n"
        headers = headers + "\tinclude/mpc_solver/"+ system +"/"+ name +"solver.h \n"
    ...
    cmake_globalvars_file.write("# This file is automatically generated \n" +\
                    "# Time of creation:" + dt_string + "\n" +\
                    "set(SYSTEM \"" + system + "\") \n" +\
                    "set(LAYERS \"" + str(len(solver_names)) + "\") \n" +\
                    "list(APPEND MODULES " +\
                    modules + ")\n")
