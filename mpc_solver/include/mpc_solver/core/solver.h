...
template<class TParam, class TOutput, class TInfo, class TMem, class TExtfunc, class TFile, class TInt>
...
public:
	...
	TInt (*solver_solve_func_)(TParam*, TOutput*, TInfo*, TMem*, TFile*, TExtfunc);
    ...
    Solver(double dt,
		   ...
		   std::vector<int> constants_ndims,
		   std::vector<int> constants_sizes,
		   std::vector<int> constants_start_idc,
		   std::vector<double> constants,
		   int n_par,
		   std::vector<std::string> pars_names,
		   int n_par_objectives,
		   std::vector<std::string> par_objectives_names,
		   int n_par_weights,
		   std::vector<std::string> par_weights_names,
		   int n_discs,
		   ...
		   size_t (*solver_get_memory_size)(),
		   TExtfunc solver_extfunc,
		   TInt (*solver_solve_func)(TParam*, TOutput*, TInfo*, TMem*, TFile*, TExtfunc)):
		SolverBase(dt,
				   n,
				   n_init,
				   nu,
		...
		solver_memory_handle_(solver_memory_handle((char*)malloc(solver_get_memory_size()), solver_number ,solver_get_memory_size())),
	...
	/**
	 * @brief Function for returning the boolean value of constant with name constant_name
	 * 
	...
	bool getConstantScalarBool(std::string constant_name) override
	{
		...
		if (constant_idx == -1)
		{
			std::cerr << "\033[31m[MPC solver] Constant with name " << constant_name << " not found! Exiting. \033[0m" << std::endl;
			exit(1);
		}
	...
	/**
	 * @brief Function for returning the integer value of constant with name constant_name
	 * 
	 * @note Exit the code if the constant is not found
	 * 
	 * @param constant_name 
	...
	/**
	 * @brief Function for returning the double value of constant with name constant_name
	 * 
	 * @note Exit the code if the constant is not found
	 * 
	 * @param constant_name 
	...
	/**
	 * @brief Function for returning an Eigen::VectorXd with values of constant with name constant_name
	 * 
	 * @param constant_name 
	...
	Eigen::VectorXd getConstantVectorDouble(std::string constant_name) override
	{
		...
		return constants_.segment(constants_start_idc_[constant_idx], constants_sizes_[size_idx]);
	}

	/**
	 * @brief Function for returning an Eigen::MatrixXd with values of constant with name constant_name
	 * 
	 * @param constant_name 
	...
	Eigen::MatrixXd getConstantMatrixDouble(std::string constant_name) override
	{
		...
		return Eigen::Map<Eigen::MatrixXd>(&constants_.segment(constants_start_idc_[constant_idx], n_rows * n_cols).data()[0], n_rows, n_cols);
	}

	/**
	 * @brief Function for returning the indexes (row) of the names attached to the solver state input data
	 * 
	 * @param find_names these are the names that we have to find in the stored names
	 * @return indexes of the names in order of the given vector
	 */
	std::vector<int> returnDataPositionsState(std::vector<std::string> find_names) override
	{
		return returnDataPositions(find_names, stage_vars_names_);
	}

	/**
	 * @brief Function for returning the indexes (row) of the names attached to the solver objective input data
	 * 
	 * @param find_names these are the names that we have to find in the stored names
	 * @return indexes of the names in order of the given vector
	 */
	std::vector<int> returnDataPositionsObjective(std::vector<std::string> find_names) override
	{
	 ...
	 * @return indexes of the names in order of the given vector
	 */
	std::vector<int> returnDataPositionsConstrain(std::vector<std::string> find_names) override
	{
	...
	*/
	...
	/**
	 * @brief Function for loading the weights into solver struct into all timesteps.
	 * This only needs to be done initially or when the weights change through reconfigure callback.
	 */
	void loadWeightsParams() override
	{
		...
		int size = npar_ * nbar_;
		int i = 0;
		for (double *ptr=ptr_start + n_par_objectives_; ptr < ptr_start + size; ptr++)
		{
			*ptr = par_weights_(i);
			i++;
	...
	void loadConstrainParams() override
	{
		...
		for (double *ptr=ptr_start + n_par_objectives_ + n_par_weights_; ptr < ptr_start + size; ptr++)
		{
	...
	/**
	 * @brief Function for loading all parameters, this could be used if parameter structure is stored
	 * for a warm start for the solver. This should not be used as the main way to update the parameters
	 */
    ...
    /**
	 * @brief Reset all parameters in the forces parameter list to zero (input to the solver)
	*/
	...
	/**
	 * @brief Copy all the solver output to the parameter list (input to the solver)
	 * and store the solver output into a matrix that can be used in the main MPC
	 * 
	...
	*/
	...
	/**
	 * @brief run the solver for one timestep and if the solver was succesfull, store the previous 
	 * 
	 * @param time_shift The timeshift we use to put the output states into the next iteration
	...
	int runSolverStep() override
	{
		...
		if (print_solver_data_ && print_solver_param_){printSolverParam();};

		int exit_code = solver_solve_func_(&forces_params_, &forces_output_, &forces_info_, solver_memory_handle_, stdout, solver_extfunc_);

		if (print_solver_data_ && print_solver_output_){printSolverOutput();};
		if (print_solver_data_ && print_solver_info_){printSolverInfo();};
	...
	*/
	void printSolverParam()
	{
		...
		double* ptr_solver_all_parameters = (double*)&forces_params_.all_parameters;
		printTableParams(12, ptr_solver_x0, ptr_solver_xinit, ptr_solver_all_parameters, sizes, nbar_, 5, sections, stage_vars_names_, pars_names_);
	}
	...
	*/
	void printSolverOutput()
	{
		...
		printTableOutput(12, ptr_solver_struct, sizes, nbar_, 5, sections, stage_vars_names_);
	}
	...
	*/
	void printSolverInfo()
	{
		...
		solver_debug_params = {"it", "it2opt", "res_eq", "res_ineq", "rsnorm", "rcompnorm", "pobj",
							   "dobj", "dgap", "rdgap", "mu", "mu_aff", "sigma", "lsit_aff", 
							   "lsit_cc", "step_aff", "step_cc", "solvetime", "fevalstime"};
	...
	*/
	...
	{
		if (print_solver_param_) {printSolverParam(); };
		if (print_solver_output_) {printSolverOutput(); };
		if (print_solver_info_) {printSolverInfo(); };
	}
};
